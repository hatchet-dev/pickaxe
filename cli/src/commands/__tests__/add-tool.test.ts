...
const mockedProcessTemplate = processTemplate as jest.MockedFunction<
  typeof processTemplate
>;
const mockedGetTemplatePath = getTemplatePath as jest.MockedFunction<
  typeof getTemplatePath
>;
const mockedUpdateBarrelFile = updateBarrelFile as jest.MockedFunction<
  typeof updateBarrelFile
>;
  ...
  describe("createTool", () => {
    it("should create tool with correct next steps", async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        "1. Define your input and output schemas in the tool file"
      );
      expect(mockConsoleLog).toHaveBeenCalledWith(
        "2. Implement the tool logic in the fn function"
      );
      expect(mockConsoleLog).toHaveBeenCalledWith(
        "3. Import and add the tool to your agent's toolbox"
      );
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining("src/tools")
      );
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining("my-tool.tool.ts")
      );
    });
