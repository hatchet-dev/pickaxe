...
export async function create(projectName?: string) {
  try {
    ...
    await processTemplate(
      { type: 'local', path: templatePath },
      config,
      { outputDir, force: true }
    );
    ...
    console.error('❌ Failed to create project:', error instanceof Error ? error.message : 'Unknown error');
    process.exit(1);
  }
}
...
async function getProjectConfig(initialName?: string): Promise<ProjectConfig> {
  const questions = [
    ...
    },
    {
      ...
      initial: (prev: string) => `A TypeScript project named ${prev}`
    },
    {
      type: 'text' as const,
      name: 'author',
    ...
    },
    {
      ...
      choices: [
        { title: 'TypeScript', value: 'typescript', description: 'Basic TypeScript project with Jest testing' }
      ],
      initial: 0
    }
  ];
