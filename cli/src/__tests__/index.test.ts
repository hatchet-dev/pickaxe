...
describe('CLI working directory functionality', () => {
  ...
  describe('working directory validation', () => {
    it('should change working directory when valid directory is provided', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'create', 'test-project']);

      expect(mockProcessChdir).toHaveBeenCalledWith(path.resolve(testDir));
      expect(mockedCreate).toHaveBeenCalledWith('test-project', {}, expect.any(Object));
    });
    ...
    it('should resolve relative paths to absolute paths', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', relativeDir, 'create', 'test-project']);
    ...
    it('should handle tilde expansion for home directory', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', homeDir, 'create', 'test-project']);
  ...
  describe('error handling for invalid directories', () => {
    it('should exit with error when directory does not exist', async () => {
      ...
      try {
        await program.parseAsync(['node', 'pickaxe', '-C', nonExistentDir, 'create', 'test-project']);
      } catch (error) {
        // Expected to throw due to process.exit mock
      }

      expect(mockConsoleError).toHaveBeenCalledWith(`Error: Directory '${path.resolve(nonExistentDir)}' does not exist`);
    ...
    it('should exit with error when path is not a directory', async () => {
      ...
      try {
        await program.parseAsync(['node', 'pickaxe', '-C', filePath, 'create', 'test-project']);
      } catch (error) {
        // Expected to throw due to process.exit mock
      }

      expect(mockConsoleError).toHaveBeenCalledWith(`Error: '${path.resolve(filePath)}' is not a directory`);
      expect(mockProcessExit).toHaveBeenCalledWith(1);
      expect(mockedCreate).not.toHaveBeenCalled();
    });
  });
  ...
  describe('integration with different commands', () => {
    ...
    it('should work with create command', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'create', 'my-project']);

      expect(mockProcessChdir).toHaveBeenCalledWith(path.resolve(testDir));
      expect(mockedCreate).toHaveBeenCalledWith('my-project', {}, expect.any(Object));
    });
    ...
    it('should work with add agent command', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'add', 'agent', 'my-agent', '-m', 'gpt-4']);

      expect(mockProcessChdir).toHaveBeenCalledWith(path.resolve(testDir));
      expect(mockedAddAgent).toHaveBeenCalledWith('my-agent', { model: 'gpt-4' }, expect.any(Object));
    });
    ...
    it('should work with add tool command', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'add', 'tool', 'my-tool', '-c', 'utility']);

      expect(mockProcessChdir).toHaveBeenCalledWith(path.resolve(testDir));
      expect(mockedAddTool).toHaveBeenCalledWith('my-tool', { category: 'utility' }, expect.any(Object));
    });
  ...
  describe('flag variations', () => {
    ...
    it('should work with short flag -C', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'create', 'project']);

      expect(mockProcessChdir).toHaveBeenCalledWith(path.resolve(testDir));
      expect(mockedCreate).toHaveBeenCalledWith('project', {}, expect.any(Object));
    });
    ...
    it('should work with long flag --cwd', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '--cwd', testDir, 'create', 'project']);
  ...
  describe('order independence', () => {
    ...
    it('should work when -C flag is at the beginning', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'add', 'agent', 'my-agent']);
    ...
    it('should work when -C flag is before subcommand', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', testDir, 'add', 'tool', 'my-tool']);
  ...
  describe('security warnings', () => {
    ...
    it('should warn when running in home directory', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', homeDir, 'create', 'project']);

      expect(mockConsoleWarn).toHaveBeenCalledWith(`⚠️  Warning: You are about to run pickaxe in your home directory: ${homeDir}`);
      expect(mockConsoleWarn).toHaveBeenCalledWith('   This will create project files directly in your home directory.');
      expect(mockConsoleWarn).toHaveBeenCalledWith('   Consider using a dedicated workspace directory like ~/workspace or ~/projects.');
    });
    ...
    it('should warn when running in system directories', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', systemDir, 'create', 'project']);

      expect(mockConsoleWarn).toHaveBeenCalledWith(`⚠️  Warning: You are about to run pickaxe in a system directory: ${systemDir}`);
      expect(mockConsoleWarn).toHaveBeenCalledWith('   This could create project files in a system location.');
      expect(mockConsoleWarn).toHaveBeenCalledWith('   Consider using a dedicated workspace directory instead.');
    });
    ...
    it('should warn for /etc directory', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', etcDir, 'create', 'project']);

      expect(mockConsoleWarn).toHaveBeenCalledWith(`⚠️  Warning: You are about to run pickaxe in a system directory: ${etcDir}`);
    });
    ...
    it('should not warn for regular workspace directories', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', workspaceDir, 'create', 'project']);
    ...
    it('should resolve symlinks before checking for sensitive directories', async () => {
      ...
      await program.parseAsync(['node', 'pickaxe', '-C', symlinkPath, 'create', 'project']);

      expect(mockConsoleWarn).toHaveBeenCalledWith(`⚠️  Warning: You are about to run pickaxe in a system directory: ${realSystemPath}`);
    });
